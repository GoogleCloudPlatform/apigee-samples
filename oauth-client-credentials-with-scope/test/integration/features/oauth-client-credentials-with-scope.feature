...
Scenario: Using a valid OAuth Access Token with write scope and performing a GET operation
  Given I set authorization header to "Bearer `accessToken`"
  When I GET /resource
  Then response code should be 200
  And response body path $.status should be success

Scenario: Using a valid OAuth Access Token with write scope and performing a POST operation
  Given I set authorization header to "Bearer `accessToken`"
  When I POST to /resource
  Then response code should be 200
  And response body path $.status should be success
