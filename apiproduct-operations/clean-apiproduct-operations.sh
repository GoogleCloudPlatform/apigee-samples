...
delete_product() {
        ...
        if apigeecli products get --name "${product_name}" --org "$PROJECT" --token "$TOKEN" --disable-check >>/dev/null 2>&1
        then
...
delete_developer() {
        ...
        if apigeecli developers get --email "${developer_email}" --org "$PROJECT" --token "$TOKEN" --disable-check >>/dev/null 2>&1
        then
...
delete_apiproxy() {
        ...
        if apigeecli apis get --name "$proxy_name" --org "$PROJECT" --token "$TOKEN" --disable-check >/dev/null 2>&1
        then
                OUTFILE=$(mktemp /tmp/apigee-samples.apigeecli.out.XXXXXX)
                if apigeecli apis listdeploy --name "$proxy_name" --org "$PROJECT" --token "$TOKEN" --disable-check >$OUTFILE 2>&1
                then
