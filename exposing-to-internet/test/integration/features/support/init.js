...
if (!process.env.RUNTIME_HOST_ALIAS) {
  
  console.log();
  console.log('Environment variable $RUNTIME_HOST_ALIAS must be set before the tests can be run.');
  console.log();
  ...
  before(function () {
    this.apickli = new apickli.Apickli(
      "https",
      process.env.RUNTIME_HOST_ALIAS
    );
  });
}