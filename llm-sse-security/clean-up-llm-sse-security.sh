...
DEVELOPER_ID=$(apigeecli developers get --email llm-sse-security-developer@acme.com --org "$PROJECT_ID" --token "$TOKEN" --disable-check | jq .'developerId' -r)
apigeecli apps delete --id "$DEVELOPER_ID" --name llm-sse-security-app --org "$PROJECT_ID" --token "$TOKEN"

echo "Deleting Developer"
apigeecli developers delete --email llm-sse-security-developer@acme.com --org "$PROJECT_ID" --token "$TOKEN"

echo "Deleting API Products"
apigeecli products delete --name llm-sse-security-product --org "$PROJECT_ID" --token "$TOKEN"

echo "Deleting KVM"
apigeecli kvms delete -n llm-sse-security-config --env "$APIGEE_ENV" --org "$PROJECT_ID" --token "$TOKEN"

delete_api "llm-sse-security"
